...
{
    [MathFunction("HSLToRGB")]
    public class HSLToRGBFunctionGenerator : MathFunctionGenerator
    {
        ...
        public override string GenerateFunction(string type, int dimension, string[] components)
        {
            ...
            return $@"        /// <summary>Converts HSL to RGB color space.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static {vectorType} HSLToRGB({vectorType} {varName})
        {{
            float h = {normalizeH}, s = {normalizeS}, l = {normalizeL};

            if (s <= 0f)
            {{
                float gray = l{(type == "byte" ? " * 255f" : "")};
