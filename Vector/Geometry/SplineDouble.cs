...
namespace Prowl.Vector.Geometry
{
    ...
    /// </summary>
    public struct SplineDouble : IEquatable<SplineDouble>, IFormattable
    {
        /// <summary>
        ...
        public enum UpVectorMethod
        {
            ...
            FrenetFrame,
            /// <summary>Minimize twist using parallel transport.</summary>
            MinimumTwist,
        ...
        public SplineDouble(Double3[] controlPoints, SplineType type = SplineType.CatmullRom, bool closed = false, double tension = 0.0)
        {
        ...
        /// <param name="closed">Whether the spline should be closed.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SplineDouble(Double3[] controlPoints, Double3[] tangents, bool closed = false)
        {
            ControlPoints = controlPoints ?? throw new ArgumentNullException(nameof(controlPoints));
        ...
        public SplineFrame EvaluateFrame(double t, UpVectorMethod upMethod = UpVectorMethod.FixedWorldUp)
        {
            ...
            Double3 forward = Maths.Normalize(firstDerivative);
            
            // Handle degenerate case
            if (Maths.LengthSquared(forward) < double.Epsilon * double.Epsilon)
            {
            ...
            switch (upMethod)
            {
                case UpVectorMethod.FixedWorldUp:
                    {
                        Double3 worldUp = WorldUpVector.Equals(Double3.Zero) ? Double3.UnitY : Maths.Normalize(WorldUpVector);
                        right = Maths.Normalize(Maths.Cross(forward, worldUp));
                        
                        // Handle case where forward is parallel to world up
                        if (Maths.LengthSquared(right) < double.Epsilon * double.Epsilon)
                        {
                ...
                case UpVectorMethod.FrenetFrame:
                    {
                        ...
                        Double3 normal = secondDerivative - Maths.Dot(secondDerivative, forward) * forward;
                        
                        double normalLength = Maths.Length(normal);
                        if (normalLength > double.Epsilon)
                        {
                            up = normal / normalLength;
                            curvature = normalLength / Maths.Max(Maths.LengthSquared(firstDerivative), double.Epsilon);
                        }
                        else
                        {
                            ...
                            if (Maths.LengthSquared(up) < double.Epsilon * double.Epsilon)
                            {
                                Double3 arbitrary = Maths.Abs(forward.X) < (double)0.9 ? Double3.UnitX : Double3.UnitZ;
                        ...
                        Double3 crossFirstSecond = Maths.Cross(firstDerivative, secondDerivative);
                        double crossLength = Maths.Length(crossFirstSecond);
                ...
                case UpVectorMethod.MinimumTwist:
                    {
                        // Use parallel transport to minimize twist
                        up = ParallelTransportUp(t);
                        right = Maths.Cross(forward, up);
                    }
                    break;
        ...
        private Double3 ParallelTransportUp(double t)
        {
            // Simplified parallel transport - maintains previous up vector projected onto current normal plane
            // In a full implementation, this would integrate along the curve
            
            if (t <= double.Epsilon)
            {
                // Initialize with world up
                Double3 forward = Maths.Normalize(EvaluateDerivative(0.0));
                Double3 worldUp = WorldUpVector.Equals(Double3.Zero) ? Double3.UnitY : Maths.Normalize(WorldUpVector);
                Double3 up = worldUp - Maths.Dot(worldUp, forward) * forward;
                return Maths.LengthSquared(up) > double.Epsilon * double.Epsilon ? Maths.Normalize(up) : Double3.UnitY;
            }

            // Get previous frame
            const double step = (double)0.01;
            double prevT = Maths.Max(t - step, 0.0);
            SplineFrame prevFrame = EvaluateFrame(prevT, UpVectorMethod.FixedWorldUp);
            
            Double3 currentForward = Maths.Normalize(EvaluateDerivative(t));
            
            // Project previous up onto current normal plane
            Double3 projectedUp = prevFrame.Up - Maths.Dot(prevFrame.Up, currentForward) * currentForward;
            
            return Maths.LengthSquared(projectedUp) > double.Epsilon * double.Epsilon ? 
                Maths.Normalize(projectedUp) : 
                Double3.UnitY;
        }
        ...
        private Double3 Slerp(Double3 a, Double3 b, double t)
        {
            ...
            double dot = Maths.Clamp(Maths.Dot(a, b), -1.0, 1.0);
        ...
        public (Double3 Point, double Parameter) GetClosestPoint(Double3 point, int subdivisions = 100)
        {
            ...
            for (int i = 0; i <= subdivisions; i++)
            {
                ...
                double distance = Maths.LengthSquared(point - splinePoint);
            ...
            if (Type != SplineType.Linear)
            {
                for (int iteration = 0; iteration < 10; iteration++)
                {
                    ...
                    double numerator = Maths.Dot(diff, derivative);
                    double denominator = Maths.Dot(derivative, derivative);
       ...
       public AABBDouble GetBounds(int subdivisions = 100)
       {
           if (ControlPoints == null || ControlPoints.Length == 0)
               return new AABBDouble(Double3.Zero, Double3.Zero);
       ...
       public static SplineDouble CreateLinear(Double3[] points, bool closed = false)
       {
           return new SplineDouble(points, SplineType.Linear, closed);
       }
       ...
       public static SplineDouble CreateCatmullRom(Double3[] points, bool closed = false, double tension = 0.0)
       {
           return new SplineDouble(points, SplineType.CatmullRom, closed, tension);
       }
       ...
       public static SplineDouble CreateBezier(Double3[] points)
       {
           if ((points.Length - 1) % 3 != 0)
               throw new ArgumentException("Bezier splines require 3n+1 control points for n segments");

           return new SplineDouble(points, SplineType.Bezier, false);
       }
       ...
       public static SplineDouble CreateBSpline(Double3[] points, bool closed = false)
       {
           return new SplineDouble(points, SplineType.BSpline, closed);
       }
       ...
       public static SplineDouble CreateHermite(Double3[] points, bool closed = false)
       {
           return new SplineDouble(points, SplineType.Hermite, closed);
       }
       ...
       public static SplineDouble CreateHermiteWithTangents(Double3[] points, Double3[] tangents, bool closed = false)
       {
           return new SplineDouble(points, tangents, closed);
       }
       ...
       public static SplineDouble CreateCircularArc(Double3 center, double radius, double startAngle, double endAngle, int segments = 16)
       {
       ...
       public static SplineDouble CreateHelix(Double3 center, double radius, double pitch, double turns, int segments = 16)
       {
       ...
       [MethodImpl(MethodImplOptions.AggressiveInlining)]
       public bool Equals(SplineDouble other)
       {
       ...
       [MethodImpl(MethodImplOptions.AggressiveInlining)]
       public override bool Equals(object? obj) => obj is SplineDouble other && Equals(other);
       ...
       {
           return string.Format(formatProvider, "SplineDouble(Type: {0}, Points: {1}, Closed: {2})", 
               Type, ControlPointCount, IsClosed);
       }

       public static bool operator ==(SplineDouble left, SplineDouble right) => left.Equals(right);
       public static bool operator !=(SplineDouble left, SplineDouble right) => !left.Equals(right);
   }
}
