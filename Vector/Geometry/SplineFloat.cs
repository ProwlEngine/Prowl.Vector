...
namespace Prowl.Vector.Geometry
{
    ...
    /// </summary>
    public struct SplineFloat : IEquatable<SplineFloat>, IFormattable
    {
        /// <summary>
        ...
        public SplineFloat(Float3[] controlPoints, SplineType type = SplineType.CatmullRom, bool closed = false, float tension = 0f)
        {
        ...
        /// <param name="closed">Whether the spline should be closed.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SplineFloat(Float3[] controlPoints, Float3[] tangents, bool closed = false)
        {
            ControlPoints = controlPoints ?? throw new ArgumentNullException(nameof(controlPoints));
        ...
        private Float3 ParallelTransportUp(float t)
        {
            ...
            if (t <= float.Epsilon)
            {
                ...
                Float3 forward = Maths.Normalize(EvaluateDerivative(0f));
                Float3 worldUp = WorldUpVector.Equals(Float3.Zero) ? Float3.UnitY : Maths.Normalize(WorldUpVector);
                Float3 up = worldUp - Maths.Dot(worldUp, forward) * forward;
                return Maths.LengthSquared(up) > float.Epsilon * float.Epsilon ? Maths.Normalize(up) : Float3.UnitY;
            }

            // Get previous frame
            const float step = (float)0.01;
            float prevT = Maths.Max(t - step, 0f);
            SplineFrame prevFrame = EvaluateFrame(prevT, UpVectorMethod.FixedWorldUp);
            
            Float3 currentForward = Maths.Normalize(EvaluateDerivative(t));
            
            // Project previous up onto current normal plane
            Float3 projectedUp = prevFrame.Up - Maths.Dot(prevFrame.Up, currentForward) * currentForward;
            
            return Maths.LengthSquared(projectedUp) > float.Epsilon * float.Epsilon ? 
       ...
       public AABBFloat GetBounds(int subdivisions = 100)
       {
           if (ControlPoints == null || ControlPoints.Length == 0)
               return new AABBFloat(Float3.Zero, Float3.Zero);
       ...
       public static SplineFloat CreateLinear(Float3[] points, bool closed = false)
       {
           return new SplineFloat(points, SplineType.Linear, closed);
       }
       ...
       public static SplineFloat CreateCatmullRom(Float3[] points, bool closed = false, float tension = 0f)
       {
           return new SplineFloat(points, SplineType.CatmullRom, closed, tension);
       }
       ...
       public static SplineFloat CreateBezier(Float3[] points)
       {
           if ((points.Length - 1) % 3 != 0)
               throw new ArgumentException("Bezier splines require 3n+1 control points for n segments");

           return new SplineFloat(points, SplineType.Bezier, false);
       }
       ...
       public static SplineFloat CreateBSpline(Float3[] points, bool closed = false)
       {
           return new SplineFloat(points, SplineType.BSpline, closed);
       }
       ...
       public static SplineFloat CreateHermite(Float3[] points, bool closed = false)
       {
           return new SplineFloat(points, SplineType.Hermite, closed);
       }
       ...
       public static SplineFloat CreateHermiteWithTangents(Float3[] points, Float3[] tangents, bool closed = false)
       {
           return new SplineFloat(points, tangents, closed);
       }
       ...
       public static SplineFloat CreateCircularArc(Float3 center, float radius, float startAngle, float endAngle, int segments = 16)
       {
       ...
       public static SplineFloat CreateHelix(Float3 center, float radius, float pitch, float turns, int segments = 16)
       {
       ...
       [MethodImpl(MethodImplOptions.AggressiveInlining)]
       public bool Equals(SplineFloat other)
       {
       ...
       [MethodImpl(MethodImplOptions.AggressiveInlining)]
       public override bool Equals(object? obj) => obj is SplineFloat other && Equals(other);
       ...
       {
           return string.Format(formatProvider, "SplineFloat(Type: {0}, Points: {1}, Closed: {2})", 
               Type, ControlPointCount, IsClosed);
       }

       public static bool operator ==(SplineFloat left, SplineFloat right) => left.Equals(right);
       public static bool operator !=(SplineFloat left, SplineFloat right) => !left.Equals(right);
   }
}
